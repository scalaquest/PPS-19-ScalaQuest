...
/**
 * Base trait for the implementation of the [[BehaviorableModel.Behavior]] s included into
 * [[CommonBehaviors.CommonBehaviors]], provided by default by ScalaQuest Core.
 */
trait StdCommonBehaviorsBase extends BehaviorableModel with CommonBehaviors with CommonItems {
   ...
   *
   * The storyteller should provide a Lens implementation as an implicit, as this implementation
   * exploits this mechanism internally.
   * @return
   *   A [[Lens]] to access the bag of the user.
   */
   ...
   * exploits this mechanism internally.
   * @return
   *   A [[Lens]] to access the Item configuration of each Room.
   */
   ...
   * exploits this mechanism internally.
   * @return
   *   A [[Lens]] to access the current Room.
   */
  implicit def currRoomLens: Lens[S, Room]

  /**
   * This implicit class "automagically" adds some utility methods to perform different checks into
   * the [[Model.State]] object.
   * @param state
   *   The [[Model.State]] to whom supply the utils.
   */
  implicit class StateUtils(state: S) {
     ...
     *   The [[Item]] to check.
     * @return
     *   True if the [[Item]] is contained into the bag, False otherwise.
     */
     ...
     *   The [[Item]] to check.
     * @return
     *   True if the [[Item]] is contained into the current Room, False otherwise.
     */
    def isInCurrentRoom(item: I): Boolean =
      state.game.itemsInRooms.get(state.game.player.location).exists(_ contains item)

    /**
     * The "scope" it the set of [[Item]] s currently reachable by the player, e.g. all the items in
     * the current Room or into the bag.
     * @param item
     *   The Item to check.
     * @return
     ...
     * @param maybeReaction
     *   An [[Option]] that could contain a [[Reaction]].
     * @return
     *   'this' if maybeReaction is None, 'this' modified by the given [[Reaction]], otherwise.
     */
