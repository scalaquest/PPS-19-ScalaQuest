...
import io.github.scalaquest.core.model.common.Actions.{GoNorth, GoSouth}
    ...
    "Applied to a Ground" when {
      ...
      "A directional Action is provided" should {
        "move the player in the designed Room, if defined" in {
          for {
            react    <- SimpleGround.use(GoNorth, simpleState) toRight fail("Reaction not generated")
        ...
        "not move the player, if a designed room is not defined" in {
          assert(SimpleGround.use(GoSouth, simpleState).isEmpty, "A reaction has been generated")
        }
      }
    }
