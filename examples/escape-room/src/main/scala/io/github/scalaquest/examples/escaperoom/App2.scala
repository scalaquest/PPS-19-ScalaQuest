...
object Model {
  ...
  def game: SQuest[SimpleState] = new SQuest[SimpleState] {
    override def send(input: String)(state: SimpleState): Either[String, SimpleState] = for {
      dir <- input match {
  ...
  def pusher: MessagePusher = (notifications: Seq[Message]) =>
    notifications map {
...
object App2 extends zio.App {
  ...
  override def run(args: List[String]): URIO[zio.ZEnv, ExitCode] = CLI[SimpleState](
    state,
    game,
    pusher
  ).start.exitCode
